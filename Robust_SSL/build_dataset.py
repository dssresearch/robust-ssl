...
parser.add_argument("--dataset", "-d", default="mnist", type=str, help="dataset name : [mnist, fashion_mnist]")
parser.add_argument("--nlabels", "-n", default=100, type=int, help="the number of labeled data")
args = parser.parse_args()

COUNTS = {
    "svhn": {"train": 73257, "test": 26032, "valid": 7326, "extra": 531131},
    "cifar10": {"train": 50000, "test": 10000, "valid": 5000, "extra": 0},
     "imagenet_32": {"train": 1281167, "test": 50000, "valid": 50050, "extra": 0, },
     "mnist": {"train": 60000, "test": 10000, "valid": 10000, "extra": 0, },
     "fashion_mnist": {"train": 60000, "test": 10000, "valid": 10000, "extra": 0, },
     "not_mnist": {"train": 60000, "test": 10000, "valid": 10000, "extra": 0, },
}
...
def _load_mnist():
    ...
    for train in [True, False]:
        ...
        tv_data = datasets.MNIST(_DATA_DIR, train, download=True)
        # tv_data.data = (tv_data.data/255. - 0.5)/0.5
        data = {}
        data["images"] = tv_data.data[:, None, :, :]
...
def _load_fashion_mnist():
    splits = {}
    for train in [True, False]:
        tv_data = datasets.FashionMNIST(_DATA_DIR, train, download=True)
        data = {}
        data["images"] = tv_data.data[:, None, :, :]
        data["labels"] = np.array(tv_data.targets)
        splits["train" if train else "test"] = data
...
if args.dataset == "svhn":
    ...
    train_set, test_set = _load_fashion_mnist()
