...
u_train_dataset = dataset_cfg["dataset"](args.root, args.n_label, "u_train_fashion_ood_{}".format(args.ood_ratio))
...
for p in range(5):
    ...
    # aaaa = np.ones([20, 1])
    aaaa = np.ones([20, 1]) * args.w_initial
    # aaaa[ood_idx] = 0.0
    ...
    for l_data, u_data, v_data in zip(l_loader, u_loader, val_loader_in_train):
        ...
        # for parameter in meta_model.params():
        #     parameter.requires_grad = False
        # for parameter in meta_model.linear2.params():
        ...
        if args.last:
            grads = torch.autograd.grad(loss_meta, meta_model.linear2.params(), create_graph=True)
            # grads = torch.autograd.grad(loss_meta, meta_model.params(), create_graph=True)
            meta_model.update_params_last(lr_inner=alg_cfg["lr"], source_params=grads)
        else:
            grads = torch.autograd.grad(loss_meta, meta_model.params(), create_graph=True)
